...
TRANSFORMERS_MODELS_TO_ADAPTER_TYPE_MAPPING = {
    ...
    "gpt2": {"attn.c_proj":"output_adapter","mlp.c_proj":"mh_adapter"},
    ...
    def _find_and_replace(self):
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                elif isinstance(target, transformers.pytorch_utils.Conv1D):
                    if adapter_type == "mh_adapter":
                        new_module = Linear(target.weight.shape[0], target.weight.shape[0], bias=bias, **kwargs)
                    elif adapter_type == "output_adapter":
                        new_module = Linear(target.weight.shape[1], target.weight.shape[1], bias=bias, **kwargs)
                    elif adapter_type == "parallel_adapter":
                        new_module = Linear(target.weight.shape[0], target.weight.shape[1], bias=bias, **kwargs)
...
class Linear(nn.Linear, AdapterLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.adapter_type == "output_adapter":
            self.Conv1d_attn = transformers.pytorch_utils.Conv1D(1600,1600)
        if self.adapter_type == "mh_adapter":
            self.Conv1d_mlp = transformers.pytorch_utils.Conv1D(1600,6400)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.adapter_type == "mh_adapter":
                ...
                self.Conv1d_attn.weight = self.weight
                x = self.Conv1d_attn(x)
